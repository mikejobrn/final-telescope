    ...
    render() {
        ...
        if (tags) {
            tagHtml = `<div class="flex gap-1 items-center">` + tags.split(',').map(tag => {
                ...
                if (trimmed === '🔒') return `<span class="material-symbols-rounded text-[15px]">lock</span>`;
                if (trimmed === '🟡') return `<span class="material-symbols-rounded text-[15px] text-yellow-500">radio_button_checked</span>`;
                if (trimmed === '⏰') return `<span class="material-symbols-rounded text-[15px]">schedule</span>`;
                if (trimmed === '🌧️') return `<span class="material-symbols-rounded text-[15px]">rainy</span>`;
                return `<span class="text-[11px] font-medium">${trimmed}</span>`;
            }).join('') + `</div>`;
        } else {
        ...
        if (isActive) {
            ...
            <div class="relative h-2 -ml-2 mb-8 mt-2">
                ...
                <div class="absolute left-1/2 -translate-x-1/2 bottom-full mb-1 ${themeBgColorObj} text-white text-[11px] font-bold px-3 py-1 rounded-full shadow-sm z-30 current-time-bubble animate-pulse shadow-primary/50">
                    ${formattedTime} Agora
                </div>
            </div>
                ...
                <div class="flex-1 ${themeCardBg} active-glow ${themeShadow} p-4 rounded-2xl -mt-3 border ${themeBorder}">
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <div class="flex items-center gap-2">
                             <span class="${themeTextColor} font-bold text-[13px] bg-white/50 px-2 py-0.5 rounded-md">${time}</span>
                             ${tagHtml}
                        </div>
                    </div>
                    <p class="text-slate-900 font-bold text-lg leading-tight mt-2">${desc}</p>
                    ${subDesc ? `<p class="text-[13px] ${themeTextColor} opacity-90 mt-1 font-medium flex items-center gap-1">${subDesc}</p>` : ''}
                </div>
            </div>`;
        } else if (isPast) {
            // Past item
            ...
            <div class="relative flex gap-5 items-start opacity-50 transition-opacity duration-300">
                ...
                <div class="flex-1 pb-4">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-slate-500 font-bold text-[13px]">${time}</span>
                        ${tagHtml}
                    </div>
                    <p class="text-slate-700 font-bold text-base leading-tight mt-1">${desc}</p>
                    ${subDesc ? `<p class="text-[11px] text-slate-500 italic mt-1">${subDesc}</p>` : ''}
                </div>
            </div>`;
        } else {
            // Future item
            ...
            <div class="relative flex gap-5 items-start">
                ...
                <div class="flex-1 pb-4">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-slate-400 font-bold text-[13px]">${time}</span>
                        ${tagHtml}
                    </div>
                    <p class="text-slate-700 font-bold text-base mt-1">${desc}</p>
                    ${subDesc ? `<p class="text-[11px] text-slate-400 mt-1">${subDesc}</p>` : ''}
                </div>
            </div>`;
        }
